...
export const NavbarC = () => {
  ...
  const handleLoginOut = (e) =>{
    ...
    const usuarioFil = usuariosLS.find((user)=> user.id === usuarioLogueado.id)
  ...
  return (
      <Navbar expand="lg" bg="black" data-bs-theme="dark">
        <Container>
          <Navbar.Brand as={Link} to={usuarioLogueado ? (usuarioLogueado.rol === 'usuario' ? '/user' : '/admin') : '/'}>Logo</Navbar.Brand>
          <Navbar.Toggle aria-controls="navbarSupportedContent" />
          <Navbar.Collapse id="navbarSupportedContent">
            <Nav className="me-auto">
            <Nav.Link 
            as={Link} 
            to={usuarioLogueado ? (usuarioLogueado.rol === 'usuario' ? '/user' : '/admin') : '/'}  
            className={location.pathname === '/' ? 'active' : ''}
          >
            Inicio
          </Nav.Link>
              {usuarioLogueado && usuarioLogueado.rol === 'admin' ? (
                <Nav.Link as={Link} to="/panel-peliculas" className={location.pathname === '/panel-peliculas' ? 'active' : ''}>
                  Panel de Películas
                </Nav.Link>
              ) : (
            ...
            {usuarioLogueado ? (
              <Nav>
                <Nav.Link as={Link} to="#" className={location.pathname === '/iniciar-sesion' ? 'active' : ''} onClick={handleLoginOut}>
                  Cerrar Sesión
                </Nav.Link>
              </Nav>
            ) : (
